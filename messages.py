...
async def onHelpMessage(message):
    ...
    await EmbedBuilder.BuildEmbed(
        ...
        "https://64.media.tumblr.com/58a7faae192fe804e0b0a74a105124e8/448b302783b0d2a5-00/s540x810/3aa732a81923fafcae364c16fe38218d39a1198c.gif",
        0xfe3ee4, "🎀 Someone asked for help? 🎀", None, None)


async def GetAvatar(message, member, client):
    if (member == "null"):
...
async def RandomVocaloid(message):
    ...
    randomChoice = [
        "Is the best for sure <3", "Is the worst sorry",
        "Well i dont know about this vocaloid one"
    ]

    await message.channel.send(random.choice(vocaloidChoice))
    await message.channel.send(
        random.choice(randomChoice) + f" {message.author.display_name}")
